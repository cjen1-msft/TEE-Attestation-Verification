...
- Native CBOR, COSE, and CACI use `TavCborHandle` and the generic API in `tav/cbor.h`. C# uses that API while preserving owned input and public kind values. C++ CBOR failures use `tav::Exception` and `tav::ErrorCode`.
